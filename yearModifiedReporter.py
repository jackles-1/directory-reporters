...
def print_types(m_year_list, m_writer):

    m_writer.writerow(["Year Last Modified", "Number of Files", "Size (B)", "Size (KB)", "Size (MB)", "Size(GB)", "", "Report run on : " + current_time])

    for m in m_year_list:
        sizes = bytes(m[1])
        new_list = [m[0], m[2], m[1], sizes["kb"], sizes["mb"], sizes["gb"]]
        m_writer.writerow(new_list)
#------------------------------------------------------------------------------
#function to check if the output file type entered is valid
def type_check(valid_name):
    output_name = easygui.filesavebox("Where would you like to save the summaries? Please save as a .csv file", title, ".csv")
    try:
...
def create_FileType(file_path, m_year_list):
    ...
    m_date = datetime.datetime.fromtimestamp(os.stat(file_path)[8])
    m_year = m_date.year
    ...
    for sublist in m_year_list:
        if sublist[0] == m_year:
            sublist[2] = sublist[2] + 1
            sublist[1] = sublist[1] + size
            m_set = True
            return
    m_year_list.append([m_year, size, 1])
...
title = "Drive File Summary - Year Modified"
    ...
    while True:
        try: 
            if x[1] == "Thumbs.db":
                ...
                create_FileType(x[0], m_year_list)
                break
        except WindowsError:
            break
#------------------------------------------------------------------------------
            ...
            while True:
                try: 
                    if f == "Thumbs.db":
                        ...
                        create_FileType(file_path, m_year_list)
                        break
                except WindowsError:
                    break
...
m_writer = csv.writer(output_file, delimiter = ",")
print_types(m_year_list, m_writer)
